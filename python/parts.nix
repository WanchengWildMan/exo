  ...
  mkPythonSet = { pkgs, lib, self' }:
    let
      ...
      pyprojectOverlay = workspace.mkPyprojectOverlay {
        ...
        dependencies = { exo = lib.optionals isLinux [ uv_extra ]; };
      };
      ...
      buildSystemsOverlay = final: prev: { }
        ...
        nvidia-cusparse = prev.nvidia-cusparse.overrideAttrs (old: {
          buildInputs = old.buildInputs ++ [ (builtins.map lib.getLib cudaLibs) ];
          autoPatchelfIgnoreMissingDeps = [ "libcuda.so.1" ];
        });
      ...
      editablePythonSet = pythonSet.overrideScope
        editableOverlay
      ;
      mkVenv = name: members: (pythonSet.mkVirtualEnv "${name}-env" members).overrideAttrs (_: { venvSkip = [ "lib/python${python.pythonVersion}/site-packages/mlx/share/cmake/*" ]; });
      execGL = cmd: "exec " + lib.optionalString cudaSupport "${lib.getExe pkgs.nix-gl-host} " + cmd;
    in
    {
      inherit pythonSet editablePythonSet;
      mkPythonScript = members: name: path: pkgs.writeShellApplication {
        inherit name;
        runtimeEnv = {
          EXO_DASHBOARD_DIR = self'.packages.dashboard;
          EXO_RESOURCES_DIR = inputs.self + /resources;
        };
        runtimeInputs = [ (mkVenv name members) ] ++ lib.optionals isDarwin [ pkgs.macmon ];
        text = execGL ''python ${path} "$@"'';
      };
      mkExo = name: members: pkgs.writeShellApplication {
        inherit name;
        runtimeEnv = {
          EXO_DASHBOARD_DIR = self'.packages.dashboard;
          EXO_RESOURCES_DIR = inputs.self + /resources;
...
in
{
  perSystem =
    ...
    {
      packages = {
        inherit exo exo-cuda-12 exo-cuda-13;
        # for devShell
        exo-venv = exoVenv;
